...
class CF_UB_asymmetric_cosine:
    def __init__(self, urm, k=100, alpha=0.5, q=1, m=0):
        ...
        print("Start asymmetric cosine user-based model..")
        s = (urm * urm.T).tocsr()
        s_diagonal = s.diagonal()
        s = s - sps.dia_matrix((s.diagonal()[np.newaxis, :], [0]), shape=s.shape)
        s.eliminate_zeros()
        s_one = sps.csr_matrix((np.ones(s.data.shape[0]), s.indices, s.indptr), shape=(n_user, n_user),
                               dtype=np.float32)
        ...
        data, rows, cols = [], [], []
        print("Keep only k-similar user...")
        ...
        s = sps.csr_matrix((data, (rows, cols)), shape=(n_user, n_user), dtype=np.float32)
        # apply q
        s.data = s.data ** q
        # do rating
        print("Start building rating matrix...")
        # important!!! multiply for s.T not s.T.T
        r = s.T * urm
        ...
        print("Model asymmetric cosine user-based done")
        self.s = s
        self.r = r
        return
